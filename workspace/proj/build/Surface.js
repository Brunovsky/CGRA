...
function revComputeSurfaceNormal(revfunction, Z, theta, delta) {
	...
	function thetaTangent(delta) {
		return {X: -sin(theta), Y: cos(theta), Z: 0};
	}
...
{
	constructor(scene, revfunction, boundary = [-1, 1, -Math.PI, Math.PI], slices = 32, stacks = 64, coords = [0, 1, 0, 1])
	{
		super(scene);
	...
	initBuffers()
	{
        ...
        for (let j = 0; j <= stacks; ++j) { // iterate Z
        	for (let i = 0; i <= slices; ++i) { // iterate plane XY
        		let Z = b.minZ + zInc * j;
        		let r = revfunction(Z);
